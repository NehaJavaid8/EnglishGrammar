...
// Level data (same as in levelData.js)
const levelData = {
  1: {
...
// HTML template for each level
function generateLevelHTML(levelNumber, levelData) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Level ${levelNumber} - ${levelData.title}</title>
  <link rel="stylesheet" href="style.css" />
</head>
<body>
  <div class="container">
    <h1>Level ${levelNumber}: ${levelData.title}</h1>
    
    <div id="sentence">${levelData.sentence}</div>
    
    <div class="word-bank">
      ${levelData.options.map(option => `<div class='word' draggable='true'>${option}</div>`).join('')}
    </div>
    ...
    <div id="nextModal">
      <div id="nextModalContent">
        <h2>🎉 Great Job!</h2>
        <p>You completed Level ${levelNumber}!</p>
...
function generateAllLevels() {
  ...
  for (let i = 1; i <= 50; i++) {
    const levelData = levelData[i];
    if (!levelData) {
      console.log(`Warning: No data for level ${i}`);
      continue;
    }
    
    const html = generateLevelHTML(i, levelData);
    const filePath = path.join(levelsDir, `level${i}.html`);
    
    fs.writeFileSync(filePath, html);
    console.log(`Generated level${i}.html`);
  }
  
  console.log('All 50 levels generated successfully!');
}
